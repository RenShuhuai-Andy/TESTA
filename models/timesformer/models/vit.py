...
from models.timesformer.models.vit_utils import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from models.timesformer.models.helpers import load_pretrained
from models.timesformer.models.vit_utils import DropPath, to_2tuple, trunc_normal_
...
def TimeSformer(model_cfg , **kwargs):
...
class TimeSformer(nn.Module):
    def __init__(self, model_cfg , **kwargs):
        ...
        num_classes = 0
        self.pretrained = False # We don't need this because we aim to load from BLIP V-L pre-trained checkpoints instead of 2D/3D ViT checkpoints trained on vision tasks
        self.model = VisionTransformer(img_size=img_size, num_classes=num_classes, patch_size=patch_size, embed_dim=768, depth=12, 
            num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., 
            drop_path_rate=0.1, num_frames=num_frames, attention_type=attention_type, learnable_temporal_scaling=learnable_temporal_scaling, **kwargs)
