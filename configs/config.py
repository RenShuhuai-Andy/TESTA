...
from utils import str_to_bool, check_yaml_file
...
def parse_with_config(parsed_args):
    ...
    if args.config is not None:
        config_args = yaml.load(open(args.config))
...
class SharedConfigs(object):
    ...
    def parse_args(self):
        parsed_args, _ = self.parser.parse_known_args()  # change parse_args() to parse_known_args()
